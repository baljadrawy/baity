...
export function LoginForm() {
  const { step, phone, isLoading, error, botUsername, sendOtp, verifyOtp, goBack } = useOtpLogin();

  return (
    <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
      {step === 'phone' ? (
        <PhoneStep onSubmit={sendOtp} isLoading={isLoading} error={error} botUsername={botUsername} />
      ) : (
        <OtpStep
          phone={phone}
          onSubmit={verifyOtp}
